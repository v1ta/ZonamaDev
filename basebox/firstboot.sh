...
export PACKAGES="dkms build-essential linux-headers-$(uname -r) xfce4 xfce4-goodies lightdm zenity openjdk-7-jre google-chrome-stable vim vim-doc vim-scripts avahi-daemon ntp ntpdate wget unzip"
...
# Add rc.fasttrak
echo -e '## ZonamaDev Boot\n(cd ~vagrant;(git clone https://github.com/lordkator/ZonamaDev.git || (cd ZonamaDev;git stash;git pull)) 2> /dev/null;exec ZonamaDev/fasttrack/scripts/rc.fasttrack) > /dev/null 2>&1' >> /etc/rc.local
